...
int *UNLOCK_TYPE_TOTALS_START = 0x80065178;
int *UNLOCK_STATE_START = 0x80065184;

char VERIFY_UNLOCK_COST(char UNLOCK_ID) {
    int *COMPARE;
    int *UNLOCK_STATE = *(&UNLOCK_STATE_START[UNLOCK_ID * 4]);
        // We could filter out already unlocked things, but the following shouldn't incur THAT many CPU cycles
    *COMPARE = *(&UNLOCK_TYPE_TOTALS_START[*(&UNLOCK_STATE[1])]);
    if (&COMPARE == &UNLOCK_TYPE_TOTALS_START) {
        return (short) *(&UNLOCK_STATE[2]) >= (short) *COMPARE; // Gems take 2 bytes to compare unlike others
    } else {
        return (char) *(&UNLOCK_STATE[2]) >= (char) *COMPARE;
    }
}